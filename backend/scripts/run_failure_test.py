...
# Configuration
INPUT_FILE = "backend/data/universal_failures.csv"
OUTPUT_FILE = "backend/data/universal_failure_results.csv"
...
def run_test():
    ...
    print(f"Starting FAILURE TEST on {len(df)} rows...")
    ...
    for index, row in df.iterrows():
        ...
        # USE HUMAN SCORE FROM FILE (Should be 0.0)
        ...
        print(f"Processed {subject}: AI={ai_score}, Human={human_score}, Error={score_error}")
        if ai_score > 5:
             print(f"  [ALERT] HIGH SCORE ON FAILURE CASE! AI thought it was good.")
        
        # Throttle
        time.sleep(5) 

    # 4. Final Report
    print("\n=== FINAL FAILURE TEST REPORT ===")
    if os.path.exists(OUTPUT_FILE):
        ...
        if not results.empty:
            summary = results.groupby("subject").agg({
                ...
                "tone": "mean",
                "specificity": "count" # N
            }).rename(columns={"specificity": "N"})
            
            print(summary)
            
            print("\nOverall Metrics:")
            print(f"Total MAE (Goal roughly 0): {results['score_error'].mean():.2f}")
            print(f"Avg AI Score (Goal roughly 0): {results['ai_score'].mean():.2f}")
        else:
